...
class Circuit {
  ...
  _add(c) {
    ...
    if (!this.components.concat(this.labels).includes(c)) {
  ...
  toYosys() {
    const ports = {};
    const cells = {};
    for (const c of this.components.concat(this.labels)) {
      if (c instanceof Label && c instanceof Power) {
        cells[c.name] = {
          type: 'vcc',
          port_directions: {
            A: 'output',
      ...
      } else if (c instanceof Label && c instanceof Ground) {
        cells[c.name] = {
          type: 'gnd',
          port_directions: {
            A: 'input',
